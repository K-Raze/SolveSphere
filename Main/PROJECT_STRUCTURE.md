...
- **Fetch**: Authenticated users can fetch a single problem by ID or list all problems.

## 🏗️ Components

### Controllers
- **`auth.controller.js`**: Handles user and admin registration, login, and logout. Returns standard JSON responses.
- **`problem.controller.js`**: Full CRUD for problems. Create and update validate reference solutions via Judge0 before saving.

### Routes
- **`auth.routes.js`**: Maps `/user/...` endpoints to the auth controller.
- **`problem.routes.js`**: Maps `/problem/...` endpoints to the problem controller. Admin routes are protected.

### Models
- **`user.js`**: Defines the user schema (name, email, password, role). Contains methods for password hashing and JWT generation.
- **`problem.js`**: Defines the problem schema (title, difficulty, tags, test cases, startCode, referenceSolution). Has indexes for faster querying.
- **`submission.js`**: Defines the submission schema (userId, problemId, code, language, status, runtime, memory). Prepared for future use.
...
- `GET /problem/:id`: Fetch a single problem (Authenticated)
- `GET /problem/`: Fetch all problems (Authenticated)
